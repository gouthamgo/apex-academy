...
// Generate static params for all Integration topics
...
export async function generateMetadata({ params }: TopicPageProps): Promise<Metadata> {
  const topic = getTopicBySlug(params.slug, 'integration');
  ...
  return {
    title: `${topic.frontmatter.title} - Integration Patterns`,
    description: topic.frontmatter.description,
    keywords: topic.frontmatter.concepts.join(', '),
  };
}

export default async function IntegrationTopicPage({ params }: TopicPageProps) {
  const topic = getTopicBySlug(params.slug, 'integration');
  ...
  const allTopics = getTopicsBySection('integration');
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <article className="flex-1 max-w-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {previousTopic ? (
                <Link
                  href={`/integration/${previousTopic.slug}`}
              ...
              {nextTopic ? (
                <Link
                  href={`/integration/${nextTopic.slug}`}
          ...
          <aside className="lg:w-80 space-y-6">
              ...
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                ...
                <div className="space-y-4">
                  {relatedTopics.map((relatedTopic) => (
                    ...
                    >
                      <h4 className="text-sm font-medium text-gray-900 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors mb-1">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                All Integration Topics
              </h3>
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {allTopics.map((t, index) => (
                  <Link
                    key={t.slug}
                    href={`/integration/${t.slug}`}
                    className={cn(
