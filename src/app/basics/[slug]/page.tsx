...
export async function generateMetadata({ params }: TopicPageProps): Promise<Metadata> {
  const topic = getTopicBySlug(params.slug, 'basics');
  ...
  return {
    title: `${topic.frontmatter.title} - Salesforce Basics`,
    description: topic.frontmatter.description,
    keywords: topic.frontmatter.concepts.join(', '),
  };
}

export default async function BasicsTopicPage({ params }: TopicPageProps) {
  const topic = getTopicBySlug(params.slug, 'basics');
  ...
  const allTopics = getTopicsBySection('basics');
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <article className="flex-1 max-w-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {previousTopic ? (
                <Link
                  href={`/basics/${previousTopic.slug}`}
